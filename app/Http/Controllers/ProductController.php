...
class ProductController extends Controller
{
	...
	public function save(Request $request)
	{
		...
		return redirect('/product')->with(['success' => '<strong>' . $product->title . '</strong>
		Telah disimpan']);
		} catch(\Exception $e) {
	...
	public function update(Request $request, $id)
	{
		...
		{
			$product = Product::find($id); //QUERY KEDATABASE UNTUK MENGAMBIL DATA
			BERDASARKAN ID
			$product->delete(); // MENGHAPUS DATA YANG ADA DIDATABASE
			return redirect('/product')->with(['success' => '</strong>' . $product->title . '</strong>
			Dihapus']); // DIARAHKAN KEMBALI KEHALAMAN /product
		}
	}
}